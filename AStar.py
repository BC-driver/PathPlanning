...
class node:
    ...
    def __lt__(self, other):
        if self.dis + self.h < other.dis + other.h:
            return True
        else:
            return False
...
class AStar:
    ...
    def herCal(self, nodea):
        return abs(nodea.pos[0] - self.end[0]) + abs(nodea.pos[1] - self.end[1])

    def run(self):
        heapq.heappush(self.q, self.graph[self.start[0]][self.start[1]])
        self.graph[self.start[0]][self.start[1]].dis = 0
        self.graph[self.start[0]][self.start[1]].h = self.herCal(self.graph[self.start[0]][self.start[1]])
        while len(self.q) != 0:
